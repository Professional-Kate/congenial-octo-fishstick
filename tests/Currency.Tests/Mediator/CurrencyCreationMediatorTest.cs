...
namespace IdelPog.Currency.Tests.Mediator
{
    ...
    public class CurrencyCreationMediatorTest
    {
        ...
        private Mock<IStateRepository<CurrencyType, IdelPog.Currency.Contracts.Currency>> _stateRepositoryMock;
        private Mock<IDispatchOne<CurrencyCreationResponse>> _currencyCreationDispatcherMock;
        ...
        {
            _stateRepositoryMock = new Mock<IStateRepository<CurrencyType, IdelPog.Currency.Contracts.Currency>>();
            _currencyCreationDispatcherMock = new Mock<IDispatchOne<CurrencyCreationResponse>>();
        ...
        public void Positive_CreateCurrency_SingleValidCommand_CreatesCurrency(uint amount)
        {
            ...
            CurrencyCreationResponse currencyResponse = new() { CurrencyCreations = [currencyCreation] };
            _currencyCreationDTOFactoryMock.Setup(library => library.CreateFrom(new[] { currencyCreation }))
                .Returns(currencyResponse);

            _currencyCreationMediator.HandleMessages([currencyCreation]);

            _stateRepositoryMock.Verify(library => library.Add(currencyCreation.CurrencyType, It.IsAny<IdelPog.Currency.Contracts.Currency>()), Times.Once);
            _stateRepositoryMock.Verify(library => library.Contains(currencyCreation.CurrencyType), Times.Once);
            _currencyCreationDispatcherMock.Verify(library => library.Dispatch(currencyResponse), Times.Once);
        }
        ...
        public void Positive_CreateCurrency_MultipleValidCommands_CreatesCurrency()
        {
            ...
            CurrencyCreationResponse responses = new() { CurrencyCreations = [_createGold, _createGems] };
            _currencyCreationDTOFactoryMock.Setup(library => library.CreateFrom(currencyCreations))
                .Returns(responses);

            _currencyCreationMediator.HandleMessages([_createGold, _createGems]);

            _stateRepositoryMock.Verify(library => library.Add(It.IsInRange(CurrencyType.GOLD, CurrencyType.GEMS, Range.Inclusive), It.IsAny<IdelPog.Currency.Contracts.Currency>()),
                Times.Exactly(2));
            ...
            _currencyCreationDispatcherMock.Verify(library => library.Dispatch(responses), Times.Once);
        }

        [Test]
        public void Negative_CreateCurrency_CurrencyAlreadyExists_Throws()
        {
            CurrencyCreationResponse responses = new() { CurrencyCreations = [_createGold] };
            ...
            _currencyCreationMediator.HandleMessages([_createGold]);
            _currencyCreationDispatcherMock.Verify(library => library.Dispatch(responses), Times.Once);

            DuplicateEntityException exception = Assert.Throws<DuplicateEntityException>(() => _currencyCreationMediator.HandleMessages([_createGold]));
            Assert.That(exception.ID, Is.EqualTo(_createGold));

            _stateRepositoryMock.Verify(library => library.Add(CurrencyType.GOLD, It.IsAny<IdelPog.Currency.Contracts.Currency>()), Times.Exactly(1));
        ...
        public void Negative_CreateCurrency_DuplicatedRequest_Throws()
        {
            DuplicateEntityException exception =
                Assert.Throws<DuplicateEntityException>(() => _currencyCreationMediator.HandleMessages([_createGold, _createGold]));
        ...
        {
            EmptyCollectionException exception = Assert.Throws<EmptyCollectionException>(() => _currencyCreationMediator.HandleMessages([]));
