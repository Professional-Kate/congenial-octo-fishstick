...
will be updated according to the `Levelable.ExperiencePerAction`.
...
| `SkillUpdate`   | `SkillCreation` | Used to Update created Skills. Will update the `Levelable` and trigger a Level Up if applicable. |
...
| `SkillUpdateResponse`   | Successful `SkillUpdate`     | This response will detail the Levelable update of the Skill. Will dispatch one Response for each input `SkillUpdate`.          |
